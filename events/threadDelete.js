  ...
  async run(client, thread) {
    ...
    for (let follower of followers) {
      ...
      await client.pkg.axios.patch(`https://discord.com/api/webhooks/${follower.webhook.id}/${follower.webhook.token}/messages/${message.data.id}?wait=true`, {
        content: `[Original Thread Deleted]`,
        components: [
      ...
      await client.database.updateOne('follow', { guildid: follower.guildid, channelid: follower.channelid, followid: follower.followid, active: true }, {$set: {messages: follower.messages.filter(x=> x.reference.threadId != thread.id) }});
      await sleep(200);
    }
  }
}
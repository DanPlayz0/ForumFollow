...
module.exports = class extends Event {
    ...
    async run(client, thread) {
        ...
        const fourmSettings = await client.database.findOne('channels', {
            id: thread.parentId
        });
        if (!fourmSettings || fourmSettings.behavior != 'create') return;

        const followedPeople = await client.database.find('follow', {
            followid: thread.parentId
        });
        ...
        for (let follower of followedPeople) {
            ...
            await client.database.updateOne('follow', {
